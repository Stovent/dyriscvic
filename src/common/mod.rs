...
impl<T> Instruction<T> {
    pub fn new_empty(pc: T) -> Self {
        Self { pc, rd: 0, rs1: 0, rs2: 0, imm: 0 }
    }

    pub fn decode_type_fail(pc: T, opcode: u32) -> Instruction<T> {
        println!("Bad format {:#X}", opcode);
        Instruction::new_empty(pc)
    }

    pub fn decode_type_empty(pc: T, _: u32) -> Instruction<T> {
        Instruction::new_empty(pc)
    }

    pub fn decode_type_r(pc: T, opcode: u32) -> Instruction<T> {
    ...
    pub fn decode_type_i(pc: T, opcode: u32) -> Instruction<T> {
        let rd = (opcode >> 7 & 0b1_1111) as u8;
        let rs1 = (opcode >> 15 & 0b1_1111) as u8;
    ...
    pub fn decode_type_s(pc: T, opcode: u32) -> Instruction<T> {
        let rs1 = (opcode >> 15 & 0b1_1111) as u8;
        let rs2 = (opcode >> 20 & 0b1_1111) as u8;
    ...
    pub fn decode_type_b(pc: T, opcode: u32) -> Instruction<T> {
        let rs1 = (opcode >> 15 & 0b1_1111) as u8;
        let rs2 = (opcode >> 20 & 0b1_1111) as u8;
    ...
    pub fn decode_type_u(pc: T, opcode: u32) -> Instruction<T> {
        let rd = (opcode >> 7 & 0b1_1111) as u8;
        let imm = opcode as i32 & 0xFFFF_F000;
        Instruction { pc, rd, rs1: 0, rs2: 0, imm }
    }

    pub fn decode_type_j(pc: T, opcode: u32) -> Instruction<T> {
        let rd = (opcode >> 7 & 0b1_1111) as u8;
