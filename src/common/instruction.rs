...
macro_rules! declare_instruction {
    ($inst:ident, $pctype:ty) => {
        ...
        impl $inst {
            pub const fn empty(inst: Isa, pc: $pctype, _: u32) -> Self {
                Self { inst, pc, rd: 0, rs1: 0, rs2: 0, imm: 0 }
            }

            pub const fn decode_type_r(inst: Isa, pc: $pctype, opcode: u32) -> Self {
                let rd = (opcode >> 7 & 0b1_1111) as u8;
                let rs1 = (opcode >> 15 & 0b1_1111) as u8;
                let rs2 = (opcode >> 20 & 0b1_1111) as u8;
                $inst { inst, pc, rd, rs1, rs2, imm: 0 }
            }

            pub const fn decode_type_i(inst: Isa, pc: $pctype, opcode: u32) -> Self {
                let rd = (opcode >> 7 & 0b1_1111) as u8;
                let rs1 = (opcode >> 15 & 0b1_1111) as u8;
                let imm = opcode as i32 >> 20;
                $inst { inst, pc, rd, rs1, rs2: 0, imm }
            }

            pub const fn decode_type_s(inst: Isa, pc: $pctype, opcode: u32) -> Self {
                let rs1 = (opcode >> 15 & 0b1_1111) as u8;
                let rs2 = (opcode >> 20 & 0b1_1111) as u8;
                let imm = opcode as i32 >> 20 & 0xFFFF_FFE0 | opcode as i32 >> 7 & 0b1_1111;
                $inst { inst, pc, rd: 0, rs1, rs2, imm }
            }

            pub const fn decode_type_b(inst: Isa, pc: $pctype, opcode: u32) -> Self {
                let rs1 = (opcode >> 15 & 0b1_1111) as u8;
                let rs2 = (opcode >> 20 & 0b1_1111) as u8;
            ...
            pub const fn decode_type_u(inst: Isa, pc: $pctype, opcode: u32) -> Self {
                let rd = (opcode >> 7 & 0b1_1111) as u8;
                let imm = opcode as i32 & 0xFFFF_F000;
                $inst { inst, pc, rd, rs1: 0, rs2: 0, imm }
            }

            pub const fn decode_type_j(inst: Isa, pc: $pctype, opcode: u32) -> Self {
                let rd = (opcode >> 7 & 0b1_1111) as u8;
...
declare_instruction!(Instruction32, u32);
declare_instruction!(Instruction64, u64);
